...
private:
	template<typename T, typename IsVector4 = typename std::enable_if<std::is_same<T, Vector<4>>::value>::type>
	void SetUniform(const std::string& variable, const T& value);

	template<typename T, typename IsIntegral = typename std::enable_if<std::is_integral<T>::value>::type>
...
template<typename T, typename IsVector4>
